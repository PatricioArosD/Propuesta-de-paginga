...
const Strategy = () => {
  ...
  // Iconos y colores para el top grid
  const strategyIcons = ["📈", "🛠️", "🤖", "🛡️", "⚖️"];
  const headerColors = ["bg-mustard", "bg-muted-blue", "bg-olive", "bg-deep-blue", "bg-muted-blue"];

  return (
    <div className="container py-5">
      <div className="text-center mb-5">
        <h1 className="text-deep-blue fw-bold display-5 serif-font">{t.strategy.title}</h1>
        <p className="lead text-muted mx-auto col-lg-8">{t.strategy.subtitle}</p>
      </div>

      {/* Top Grid: Servicios Principales (Diseño de 4 columnas visual) */}
          ...
          <div className="col-lg-3 col-md-6" key={index}>
            <div className={`card h-100 border-0 shadow-sm text-center card-colored-header`}>
              ...
              <div className="card-body p-4 bg-white rounded-bottom">
                <h5 className="text-deep-blue serif-font mb-3">{svc.title}</h5>
                {/* Botón para expandir detalles en este layout */}
      ...
      {/* Sección "Examples" (Diseño 2x2 con cajas de texto) */}
      <div className="py-5 bg-white rounded-4 shadow-sm p-4 p-md-5 my-5">
         <h2 className="text-deep-blue serif-font mb-4 text-center">Examples & Case Studies</h2>
         <div className="row g-4">
            {/* Mapeamos los ejemplos de los primeros 4 servicios para rellenar el grid */}
                  ...
                  <div className="p-4 rounded-3 h-100" style={{ backgroundColor: idx % 2 === 0 ? 'var(--color-cream)' : '#f0f2f5' }}>
                     ...
                     <ul className="ps-3 mb-0">
                        {svc.examples.map((ex, i) => (
                           <li key={i} className="mb-2 text-deep-blue">{ex}</li>
                        ))}
                     </ul>
                  </div>
               </div>
